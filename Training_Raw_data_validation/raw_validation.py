...
class RawDataValidation:
    ...
    def schema_validation(self):
        ...
        logger = AppLogger("Training_Logs/schema_validation_logs.txt")
        ...
        try:
            ...
            logger.log("ValueError:Value not found inside schema_training.json")
            raise ValueError
        except KeyError:
    ...
    def create_dir_for_good_bad_data(self):
        ...
        logger = AppLogger("Training_Logs/general_logs.txt")
    ...
    def del_existing_good_data_training_folder(self):
    ...
    def del_existing_bad_data_training_folder(self):
    ...
    def validate_filename(self, len_date_stamp, len_time_stamp):
        ...
        self.del_existing_bad_data_training_folder()
        self.del_existing_good_data_training_folder()

        files = [f for f in listdir(self.batch_directory)]

        logger = AppLogger("Training_Logs/filename_validation_logs.txt")
                    ...
                    if '.csv' in text:
                        ...
                        if '_' in text:
                            ...
                            if text[0].isdigit() and text[0].isdigit():
                                if len(text[0]) == len_date_stamp and len(text[1]) == len_time_stamp:  # len_date_stamp and len_time_stamp
    ...
    def validate_col_len(self, no_of_columns):
        logger = AppLogger("Training_Logs/column_validation_logs.txt")
        try:
    ...
    def validate_null_columns(self):
        logger = AppLogger("Training_Logs/null_columns.txt")
        try:
            for file in listdir('Training_Raw_files_validated/Good_Raw/'):
                df = pd.read_csv(f"Training_Raw_files_validated/Good_Raw/{file}")
                count = 0
                for columns in df:
