...
class PredictionDatabaseOperation:
    """
    This class is used for validating the datatypes of the columns in our prediction data files.
    """
    def __init__(self):
        self.path = 'Prediction_Database/'
        self.bad_data_path = "Prediction_Raw_files_validated/Bad_Raw"
        self.good_data_path = "Prediction_Raw_files_validated/Good_Raw"
        self.logger = AppLogger("Prediction_Logs/database_logs.txt")
    ...
    def delete_existing_db(self, db_name):
        location = "Prediction_Database/"
        try:
            if os.path.isfile(location + db_name + ".db"):
                os.remove(location + db_name + ".db")
                self.logger.log(f"Successfully deleted the existing database: {db_name}.db from {location}")
            else:
                self.logger.log(f"No database is detected with name {db_name}.db in the direction {location}")
            self.logger.log("Exited from delete_existing_db method in PredictionDatabaseOperation class")

        except Exception as e:
            self.logger.log("Error occurred while deleting the existing database")
            self.logger.log("Exited from delete_existing_db method in PredictionDatabaseOperation class")
            raise e
